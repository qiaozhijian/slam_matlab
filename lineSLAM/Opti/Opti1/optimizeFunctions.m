...
for poseI=1:nPose
    for lineI=1:nLine
        if graph(poseI,lineI)==1
            ...
            err_i(1)=line_c_predict.sp'*l_obs;
            err_i(2)=line_c_predict.ep'*l_obs;
            ...
            J_sp=J_sp*Tcp{poseI}(1:3,1:3)*err_i(1)/err_i_norm;
            ...
            Je_aux=[+ fgz2 * lx * gz,...
                    ...
                    + fgz2 * ( gx*gx*lx + gz*gz*lx + gx*gy*ly ),...
                    + fgz2 * ( gx*gz*ly - gy*gz*lx )];
            J_pose = (Js_aux * ds + Je_aux * de)/err_i_norm;
            J_ep=[fgz2*lx*gz,fgz2*ly*gz,-fgz2*(lx*gx+ly*gy)];
            J_ep=J_ep*Tcp{poseI}(1:3,1:3)*err_i(2)/err_i_norm;
            ...
            g(poseIdx:poseIdx+5) = g(poseIdx:poseIdx+5) - J_pose' * err_i_norm;
            g(lineIdx:lineIdx+5) = g(lineIdx:lineIdx+5) - J_line' * err_i_norm;
            e = e + err_i_norm;
        end
    end
end
